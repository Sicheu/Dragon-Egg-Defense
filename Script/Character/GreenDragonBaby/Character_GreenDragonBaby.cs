...
public class Character_GreenDragonBaby : CharacterBase<FSM_GreenDragonBaby>, ISelectable
{
    ...
    public bool Selecting = false; // 선택 중인지 아닌지 나타내는 변수
    ...
    public void StartSkillInstance(SkillInstance instance)
    {
        ...
        Fsm.ChangeState(FSM_GreenDragonBabyState.FSM_GreenDragonBabyState_Skill);
    }
    ...
    public void GetDamaged()
    {
        if (ActiveSkillInstance.target != null) // target이 null인지 확인 = 타겟이 Dead 했을 때 오류 발생
        {
            ...
            if (targetSoldier != null)
            {
                ...
                targetSoldier.OnStun(ActiveSkillInstance.info.StunTime);
            }
        }
        else
        {
    ...
    public IEnumerator ReadyToMove() // 선택된 상태에서 Idle 상태와 Skill 상태를 유지하면서 언제든지 MoveToPoint 로 이동할 수 있도록 하기 위해 만든 코루틴
    {
        while (true)
        {
            ...
            {
                Fsm.ChangeState(FSM_GreenDragonBabyState.FSM_GreenDragonBabyState_MoveToPoint);
